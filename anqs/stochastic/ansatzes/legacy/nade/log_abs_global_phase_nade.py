...
class LogAbsGlobalPhaseNADE(AbstractNADE):
    def __init__(self,
                 *args,
                 depth: int = 0,
                 width: int = None,
                 ...
                 **kwargs):
        super(LogAbsGlobalPhaseNADE, self).__init__(*args, **kwargs)
        ...
        self.phase_mlp = MLP(in_num=self.qubit_num,
                             out_num=1,
    ...
    def phase(self, x):
        assert x.shape[-1] == self.qubit_num
        assert x.dtype == self.rdtype

        phases = pt.squeeze(self.phase_mlp(x), dim=-1)
